...
document.addEventListener('DOMContentLoaded', () => {
    ...
    const SECRET_ADMIN_PASSWORD = 'wjsdur1206!!@@'; // 여기에 실제 비밀번호를 설정하세요
    ...
    skillsData.forEach(skill => {
        ...
        skillItem.innerHTML = `
            ...
            <div>
                ...
                <button onclick="deleteSkill(${skill.id})" class="btn btn-small secondary">삭제</button>
            </div>
        `;
        adminSkillsList.appendChild(skillItem);
    });
}
...
function deleteSkill(id) {
    if (confirm('정말로 이 기술 스택을 삭제하시겠습니까?')) {
...
let contactInfo = JSON.parse(localStorage.getItem('portfolioContact')) || {
    ...
    location: "서울, 대한민국",
    github: "https://www.saramin.co.kr", // 사람인
    linkedin: "https://rememberapp.co.kr", // 리멤버
    twitter: "https://www.jobkorea.co.kr" // 잡코리아
};
...
function renderContactInfo() {
    const contactEmailElement = document.getElementById('contactEmailDisplay');
    const contactPhoneElement = document.getElementById('contactPhoneDisplay');
    const contactLocationElement = document.getElementById('contactLocationDisplay');
    ...
    const contactTwitterLink = document.getElementById('contactTwitterLink');
